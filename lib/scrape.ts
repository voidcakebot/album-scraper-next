...
const toAbsoluteUrl = (value: string | undefined, sourceUrl: string) => {
  if (!value) return '';
  try { return new URL(value, sourceUrl).toString(); } catch { return ''; }
};

function parseJsonLd(html: string): JsonLd[] {
  const $ = load(html); const blocks: JsonLd[] = [];
  $('script[type="application/ld+json"]').each((_, el) => {
    const raw = $(el).text(); if (!raw) return;
    try {
      const parsed = JSON.parse(raw) as unknown;
      if (Array.isArray(parsed)) parsed.forEach((i) => { if (i && typeof i === 'object') blocks.push(i as JsonLd); });
...
function extractFromJsonLd(blocks: JsonLd[]) {
  for (const block of blocks) {
    ...
    const coverImageUrl = typeof imageCandidate === 'string' ? imageCandidate : Array.isArray(imageCandidate) && typeof imageCandidate[0] === 'string' ? imageCandidate[0] : undefined;
    const byArtist = block.byArtist as JsonLd | JsonLd[] | undefined;
    const artist = block.artist as JsonLd | JsonLd[] | undefined;
    const artistCandidate = Array.isArray(byArtist) ? byArtist[0] : byArtist ?? (Array.isArray(artist) ? artist[0] : artist);
    const artistName = artistCandidate && typeof artistCandidate === 'object' ? normalizeText(String((artistCandidate as JsonLd).name ?? '')) || undefined : undefined;
...
export function parseAlbumFromHtml(html: string, sourceUrl: string): Omit<Album, 'scrapedAt'> {
  ...
  const fallbackImg = $('img').map((_, el) => $(el).attr('src')).get().map((src) => toAbsoluteUrl(src, sourceUrl)).find((src) => /^https?:\/\//i.test(src));

  const coverImageUrl = toAbsoluteUrl(ogImage, sourceUrl) || toAbsoluteUrl(twitterImage, sourceUrl) || toAbsoluteUrl(jsonLd.coverImageUrl, sourceUrl) || fallbackImg || '';
  ...
  const artistName = normalizeText(jsonLd.artistName) || normalizeText(authorMeta) || normalizeText(domArtist) || inferArtistFromTitle(ogTitle ?? titleTag ?? '') || 'Unknown Artist';
...
export async function fetchHtml(sourceUrl: string, timeoutMs: number): Promise<string> {
  ...
  try {
    const response = await fetch(sourceUrl, { headers: { 'user-agent': DEFAULT_USER_AGENT, accept: 'text/html,application/xhtml+xml' }, signal: controller.signal });
